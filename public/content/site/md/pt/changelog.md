...
## Versão 4.5.0
*(Lançada em 24 de setembro de 2026)*

A Versão 4.5.0 introduz uma evolução visual e estrutural, com a adoção total da linguagem de design Material 3 Expressive. Removemos o *glassmorphism* obsoleto e os gradientes arbitrários em favor de superfícies geométricas planas e limpas, animações fluidas e uma hierarquia de plano de fundo unificada, responsiva em qualquer dispositivo.

Além da reformulação da interface de usuário (UI), esta atualização traz um ecossistema de download totalmente novo e seguro, projetado para proteger os usuários contra APKs de terceiros, juntamente com recursos avançados de roteamento de borda (*edge-routing*), como um encurtador de URL integrado e metadados dinâmicos do Open Graph, para um compartilhamento social mais rico.

* **Material 3 Expressive UI:** Reformulamos completamente o sistema de design para aderir às diretrizes estritas do Material 3. Implementamos superfícies tonais sólidas, removemos as bordas antigas, redesenhamos a página NotFound com uma grade assimétrica e unificamos todas as interfaces (*app shells*) para um layout contínuo e imersivo.
* **Distribuição Segura de Aplicativos:** Lançamos um ambiente de download dedicado e localizado, que apresenta verificações dinâmicas de requisitos de hardware (como restrições de ARCore ou Magnetômetro) e avisos de segurança proativos, para afastar os usuários de lojas de aplicativos de terceiros potencialmente maliciosas.
* **Navegação e Layouts Fluidos:** Integramos o Framer Motion para transições de visualização extremamente suaves em dispositivos móveis, tablets e computadores. Agora, a barra de pesquisa para tablets se expande de forma inteligente para maximizar o foco, enquanto a barra lateral para computadores utiliza uma nova arquitetura de rastreamento de rolagem dupla e um deslocamento dinâmico de rodapé.
* **Roteamento Inteligente e SEO:** Implementamos um encurtador de URL personalizado por meio do Cloudflare Pages para um compartilhamento de links mais limpo (por exemplo, `/s/pc/c/...`), otimizamos totalmente as ordens de redirecionamento para obter o desempenho máximo e introduzimos a geração dinâmica de metadados do Open Graph para pré-visualizações de links precisas e ricas em plataformas sociais.
* **Núcleo e Desempenho:** Alteramos os ouvintes de eventos (*event listeners*) de redimensionamento de janela para o `requestAnimationFrame`, a fim de obter um dimensionamento de UI perfeitamente suave; pré-inicializamos portais DOM globais persistentes para eliminar falhas de renderização; e reestruturamos as consultas de mídia (*media queries*) CSS para evitar completamente o Flash of Unstyled Content (FOUC) durante as alterações da janela de visualização (*viewport*).
...
Esta atualização tem como foco aprimorar significativamente a apresentação visual e a experiência em dispositivos móveis do Visualizador de Roadmap. Introduzimos o suporte a rich text e a tabelas Markdown com o objetivo de fornecer um conteúdo mais detalhado e estruturado dentro das seções do roadmap.

*   **Novo: Renderização de Rich Text:** O Visualizador de Roadmap agora suporta totalmente a formatação Markdown em negrito e itálico nas descrições de texto, o que torna o conteúdo mais fácil de ler e enfatizar.
*   **Novo: Suporte a Tabelas Markdown:** O "Histórico de Versões" e outros dados tabulares agora podem ser renderizados diretamente a partir de tabelas Markdown. Em dispositivos móveis, essas tabelas se transformam de forma inteligente em cartões responsivos, ao passo que mantêm a sua estrutura tradicional em telas maiores.
*   **Novo: Conteúdo de Texto Geral:** As seções do roadmap foram expandidas para suportar parágrafos descritivos gerais juntamente com tarefas e itens agrupados.
*   **Aprimoramento de UI e UX: Layouts Responsivos:** O visualizador agora ajusta dinamicamente os elementos da linha do tempo, os tamanhos dos ícones e o preenchimento especificamente para dispositivos móveis, para garantir uma experiência impecável independentemente do tamanho da sua tela.
*   **Aprimoramento de UI e UX: Refinamentos Visuais:** Aplicamos extensas atualizações de estilo em todos os componentes por meio dos princípios do Material Design 3. Espere cores novas, ícones de status de roadmap atualizados e cartões de seção e áreas de contato refinadas.
*   **Nos Bastidores: Lógica de Análise Aprimorada:** O `roadmapParser` interno foi completamente reescrito para detectar com precisão os status das seções e extrair os novos tipos de conteúdo (texto geral e tabelas).

## Versão 4.1.1
*(Lançada em 15 de abril de 2026)*

Esta atualização menor inclui adições essenciais à documentação de ajuda e às seções de FAQ, e aborda especificamente a compatibilidade de hardware e o suporte de provedores regionais de clima introduzidos nas atualizações recentes do Pixel Compass.

*   **Documentação: FAQ de Incompatibilidade de Hardware:** Adicionamos instruções claras e entradas de FAQ que explicam o novo requisito estrito para um sensor de magnetômetro físico (compass). O texto também fornece instruções de reembolso manual aos usuários afetados do Pixel Compass+ e esclarece a disponibilidade contínua das versões mais antigas.
*   **Documentação: Provedores Regionais de Clima:** Adicionamos uma nova seção de "Provedores de Clima e Suporte Regional", a qual explica o mecanismo meteorológico em várias camadas (Google Weather como principal, OpenWeatherMap como alternativa), as diferenças de dados e as orientações para erros de "Região Incompatível".
*   **Documentação: Atualização das Informações de Contato:** Atualizamos o e-mail de contato principal de suporte para `suport@fertwbr.com` em todo o FAQ e na documentação de ajuda.

## Version 4.1.0
*(Released April 14, 2026)*

A versão 4.1.0 introduz novas formas poderosas de compartilhar atualizações e traz uma camada de polimento premium à interface do site. Este lançamento foca em tornar a informação mais acessível e facilmente distribuível através do nosso novo mecanismo de compartilhamento de e-mail rico, ao mesmo tempo em que entrega uma experiência de navegação mais suave e responsiva para usuários de tablets.

#### 🌐 Core Web Experience
* **New: Rich Email Sharing Engine:** Compartilhar atualizações importantes agora é simples. O site agora pode gerar embeds de e-mail HTML elegantemente estilizados diretamente do changelog e conteúdo Markdown.
  * **Cross-Client Compatibility:** Os embeds são gerados dinamicamente usando estilização inline precisa (utilizando opacidade moderna e formatação RGBA em vez de cores fixas) para garantir que apareçam perfeitamente e herdem as fontes corretas em todos os principais clientes de e-mail.
  * **Smart Topic Extraction:** O sistema agora extrai de forma inteligente tópicos e cabeçalhos Markdown, empacotando-os de maneira organizada no payload do e-mail, juntamente com um fallback confiável de área de transferência em texto simples para máxima compatibilidade.
  * **Seamless Copy Feedback:** Diga adeus aos alertas pop-up intrusivos do navegador. Copiar links ou embeds de e-mail agora aciona uma notificação toast elegante e leve para um fluxo premium ininterrupto.

* **UI & UX Polish: Tablet Navigation Overhaul:** A barra de navegação para tablets foi reconstruída do zero, afastando-se de estilos inline para uma arquitetura robusta baseada em CSS para uma responsividade superior.
  * **Refined Glass Effect:** A estética de efeito vidro (glass) característica agora está mais suave, consistente e fácil de estilizar conforme você rola a página.
  * **Intelligent Layouts:** Os rótulos de navegação agora se ocultam dinamicamente em telas de tablet mais estreitas (abaixo de 900px), priorizando o espaço de tela para o conteúdo enquanto mantém os ícones essenciais perfeitamente alinhados.

* **Global Localization: Expanded Translations:** O novo conjunto de compartilhamento por e-mail está totalmente localizado e pronto para uso global.
  * **Multi-Language Support:** Adicionadas chaves de tradução abrangentes para todos os fluxos de compartilhamento e cópia em alemão, inglês, espanhol, hindi, japonês e português.

* **Fixes & Stability: Ecosystem & Routing:** Correções essenciais para garantir que os usuários sempre cheguem ao destino correto sem fricção.
  * **Verified Store Links:** Substituídos os URLs de espaço reservado pelos links oficiais e ativos da Chrome Web Store e Firefox Add-on.
  * **Documentation Anchors:** Normalizados os metadados do índice (TOC) nas seções de Ajuda e FAQ (convertendo para kebab-case), garantindo que links diretos e a rolagem automática funcionem perfeitamente sempre.
  * **Repository Routing:** Corrigido um erro de digitação de URL na documentação do desenvolvedor para apontar corretamente para a base de código open-source do Gemini-Expressive.
