...
* **Core & Architecture: Masonry Grid Layout Engine:** Implemented a new absolute 2D bin-packing layout resolver to handle complex dynamic weather grids.
  * Replaced sequential lazy grids with coordinate-based `WeatherGridSpanResolver` for exact 1:1 pixel-perfect drag, drop, and resize gestures, now supporting dynamic resizing from all four corners.
...
* **New: Precipitation Ecosystem:** A complete overhaul of precipitation data mapping and visualization.
  ...
  * Introduced adaptive `PrecipitationBlock` routing, including new optimized Free-tier layouts (`SmallPrecipitationBlockFree`, `WidePrecipitationBlockFree`) and a universal `PrecipitationDryStateCard` fallback.
  * Built advanced timeline intelligence in `PrecipitationBlockMapper` to calculate peak volumes, start/stop times, exact timezone-aware localized markers, and precise snow detection logic.
...
* **New: Advanced Wind Blocks:** Comprehensive modularization and visual redesign of wind telemetry.
  ...
  * Integrated Beaufort scale calculations, localized gust badges, and direction vector mapping directly into the UI state, supporting expanded Free-tier layouts.
...
* **New: Daily & Hourly Forecast Enhancements:** Major visual and structural updates to predictive timelines.
  * Redesigned the `ExtraLargeDailyForecastBlock` into a dual-pane layout optimized for tablets and foldables, featuring staggered entrance animations.
  * Introduced adaptive compact modes for hourly forecast pills, scaling dimensions dynamically based on available vertical constraints.
  * Added vibrant gradient day labels ("Today") and adaptive temperature pills that smartly format text when vertical space is restricted.
...
* **UI & UX Polish: Editor Haptics & Interactions:** Introduced deep tactile feedback and intelligent touch handling for layout customization.
  ...
  * Automatically disabled interactive block elements (like charts, daily forecast paging, tabletop mode switching, and compasses) while in Edit Mode to prevent nested gesture conflicts.
...
* **UI & UX Polish: Environmental Backgrounds:** Upgraded atmospheric rendering layers and logical season resolutions.
  ...
  * Added strict override logic to force `Winter` volumetric terrain rendering during active snow conditions regardless of the calendar date, and removed unrealistic swaying effects from ground-level spring flowers.
...
* **Fixes & Stability: Data Caching & Reliability:** Addressed critical bugs affecting data accuracy, deep linking, and layout stability.
  * Fixed an issue where duplicate weather blocks failed to update their transparency state; transparency is now accurately synchronized via type-based keys instead of instance UUIDs.
  ...
  * Fixed combined text formatting errors in `AtAGlanceLayout` and ensured the detail panel reactively observes `fullWeatherData` to prevent stale rendering.
...
This release tackles critical layout and logic bugs affecting the At-a-Glance widget, ensuring cross-manufacturer compatibility for system timers and drastically improving text legibility and notification deep linking.

#### 📱 Phone
* **UI & UX Polish: Intelligent Widget Contrast (AUTO Mode):**
  * **Wallpaper Luminance Detection:** When your widget background is disabled and the font color is set to "AUTO," the app now actively scans your home screen wallpaper's luminance using the `WallpaperManager` (API 27+). It intelligently switches the text between stark black or white to guarantee maximum legibility against your specific background.
* **Fixes & Stability: Universal System Timer Support:**
  * **Cross-OEM Compatibility:** The `TimerNotificationListenerService` has been drastically expanded. It no longer relies solely on the Google DeskClock app; it now actively detects and synchronizes live timers from Samsung, OnePlus, Oppo, Huawei, and Vivo native clock apps.
  * **Pre-existing Timers:** The service will now actively scan existing notifications upon connection, ensuring timers started *before* the widget was placed are immediately picked up and displayed.
* **Fixes & Stability: Glance Widget Layout Glitches:**
  * **Text Cropping Fix:** Resolved a widespread issue where the bottom of letters (like 'y', 'g', 'p') were brutally clipped by the launcher when using heavier fonts or dynamic sizing. We restructured the `RemoteViews` hierarchy, wrapping text nodes in explicit columns with forced structural spacers to ensure the Android layout engine grants the text proper breathing room.
  * **Content Wrapping:** Explicit `wrapContentHeight()` constraints have been applied across the widget to prevent UI compression.
* **Core & Performance: Smarter Notifications & Insights:**
  * **Contextual Deep Linking:** Severe weather alerts generated by the `EnvironmentSyncWorker` (like rain, UV, or temperature drops) now contain specific deep links. Tapping the notification bypasses the home screen and launches you *directly* into the relevant UI card (e.g., jumping straight to the detailed UV index view).
  * **Calendar Stalling Fix:** The `CalendarInsightProvider` now utilizes an active 15-minute ticker loop. This fixes a bug where the widget would stubbornly hold onto an old calendar event because the internal clock wasn't re-evaluating the "now" state unless the database manually changed.

## Version 1.22.0 Beta 3.2
*(Released July 8, 2026)*

 This update focuses heavily on the underlying architecture of the Wear OS app. We've optimized how the app manages hardware sensors to drastically improve battery life, reliability, and Always-On Display (AOD) performance.

#### ⌚ Wear OS
* **Core & Performance: Smart Sensor Lifecycle Management:** We've completely overhauled when and how the app listens to your smartwatch's hardware sensors.
  * **Visibility-Aware Polling:** The app now actively tracks your vertical and horizontal navigation (`currentHorizontalPage` and `currentVerticalPage`). Sensors are instantly spun down when the compass or level UI is not actively visible on your screen, saving crucial battery life.
  * **Resume Reliability:** Introduced a deliberate 300-millisecond stabilization delay when waking the app from the background. This ensures hardware sensors have enough time to initialize properly before rendering, preventing glitched initial readings.
  * **Centralized Delegation:** Sensor state and reference counting have been modernized and moved directly into the `SensorDataManager` for a cleaner, more robust architecture.
* **Fixes & Stability: Always-On Display (AOD) Freezes:** Addressed a critical issue where the app could occasionally freeze when transitioning to the Always-On Display. The engine now properly clears its internal navigation matrices and velocity states the moment it stops listening to hardware events.
* **Fixes & Stability: Barometer Fallbacks & Elevation:**
  * **Pressure Safety:** Continued improvements for watches without physical barometers. The sensor orchestrator now seamlessly falls back to standard atmospheric pressure (`PRESSURE_STANDARD_ATMOSPHERE`) if raw data is missing, preventing calculation crashes.
  * **Sequential Fetching:** Elevation data fetches are now processed sequentially to ensure no altitude calibration requests are dropped or prematurely canceled.
* **Under the Hood: Codebase Modernization:**
  * **Duration API:** Replaced legacy, raw millisecond integers with Kotlin's modern, type-safe `Duration` API across the entire Wear OS module—spanning complication update monitors, calibration screens, and sensor orchestration.
  * **Math Optimization:** Migrated heavy sensor math calculations from legacy Java utilities to highly optimized `kotlin.math` functions.
