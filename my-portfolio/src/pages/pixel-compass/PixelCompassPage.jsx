...
export default function PixelCompassPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return params.get('page') && pixelCompassConfig.pages[params.get('page')] 
      ? params.get('page') 
      : pixelCompassConfig.defaultPage;
  });

  const [activeColor, setActiveColor] = useState(() => {
    const params = new URLSearchParams(location.search);
    const colorParam = params.get('color');
    return colorParam ? `#${colorParam.replace('#', '')}` : pixelCompassConfig.seedColor;
  });
  ...
  usePageMetadata({
    title: `${pixelCompassConfig.appName} - ${activeTab === 'index' ? 'Home' : activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}`,
    themeColor: surfaceColor,
    favicon: "https://raw.githubusercontent.com/FertwBr/PixelAssets/main/Compass/art/favicon/favicon.ico"
  });
  ...
  useEffect(() => {
    const pageConfig = pixelCompassConfig.pages[activeTab];
    ...
    loadPageContent(activeTab, pixelCompassConfig)
      .then((text) => {
        if (isMounted) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <main style={{ flex: 1, width: '100%', overflowX: 'hidden' }}>
        <AnimatePresence mode="wait">
          {activeTab === 'index' ? (
             <PixelCompassHome key="home" onNavigate={handleNavigation} strings={t} />
          ) : (
            <motion.div
                key="content"
